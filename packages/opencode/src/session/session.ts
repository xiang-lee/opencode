    ...
    const lookupByKey = Effect.fn("Session.lookupByKey")(function* (key: string) {
      ...
      const row = yield* db((d) =>
        d
          .select()
          .from(SessionTable)
          .where(and(eq(SessionTable.project_id, ctx.project.id), eq(SessionTable.session_key, value)))
...
export async function get(id: SessionID): Promise<Info> {
  return runPromise((svc) => svc.get(id))
}

export async function getByKey(key: string): Promise<Info> {
  return runPromise((svc) => svc.getByKey(key))
}

export async function messages(input: { sessionID: SessionID; limit?: number }): Promise<MessageV2.WithParts[]> {
  return runPromise((svc) => svc.messages(input))
}
...
export async function removeMessage(input: { sessionID: SessionID; messageID: MessageID }): Promise<MessageID> {
  return runPromise((svc) => svc.removeMessage(input))
}
