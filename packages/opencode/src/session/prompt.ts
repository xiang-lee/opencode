...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
    ...
    const shellImpl = Effect.fn("SessionPrompt.shellImpl")(function* (input: ShellInput) {
      const setup = yield* Effect.uninterruptible(
        Effect.gen(function* () {
          const ctx = yield* InstanceState.context
          const run = yield* runner()
          ...
          return { cmd, msg, part, run }
        }),
      )
      const { cmd, msg, part, run } = setup
    ...
    const lastShellAssistant = Effect.fnUntraced(function* (sessionID: SessionID) {
      const msg = yield* lastAssistant(sessionID)
      const parts: MessageV2.Part[] = []
      for (const part of msg.parts) {
        if (part.type === "tool" && part.tool === "bash" && part.state.status === "running") {
          const output =
            String(part.state.metadata?.output ?? "") +
            "\n\n" +
            ["<metadata>", "User aborted the command", "</metadata>"].join("\n")
          const next = yield* sessions.updatePart({
            ...part,
            state: {
              status: "completed",
              time: { ...part.state.time, end: Date.now() },
      ...
      return { info: msg.info, parts }
    })

    const shell: (input: ShellInput) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.shell")(
      function* (input: ShellInput) {
        return yield* state.startShell(input.sessionID, lastShellAssistant(input.sessionID), shellImpl(input))
      },
    )
