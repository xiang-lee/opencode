...
export const make = <A, E = never>(
  ...
  const waitShell = (done: Deferred.Deferred<A, E | Cancelled>) =>
  ...
  const startShellFiber = (
    ...
    work.pipe(
      ...
      Effect.forkIn(scope),
    ) as Effect.Effect<Fiber.Fiber<A, E>>
  ...
  const startShell = (work: Effect.Effect<A, E>) =>
    SynchronizedRef.modifyEffect(
      ...
      Effect.fnUntraced(function* (st) {
        ...
        const id = next()
        const done = yield* Deferred.make<A, E | Cancelled>()
        const fiber = yield* startShellFiber(id, done, work)
        const shell = { id, done, fiber } satisfies ShellHandle<A, E>
        return [waitShell(done), { _tag: "Shell", shell }] as const
      }),
    ).pipe(Effect.flatten)
